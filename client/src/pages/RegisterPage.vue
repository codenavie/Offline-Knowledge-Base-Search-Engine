<template>
  <section class="section auth-head">
    ...
    <p class="section-sub">Create your personal workspace</p>
  </section>
  ...
  <section class="section auth-body">
    <form class="auth-form" @submit.prevent="submit">
      ...
      <label>
        <span>Password</span>
        <input class="bau-input" v-model="password" type="password" minlength="8" required />
      </label>

      <button class="bau-btn bau-red" :disabled="loading">{{ loading ? "Creating" : "Create Account" }}</button>
    </form>

    <p v-if="message" class="message">{{ message }}</p>
    <p class="muted switcher">Already registered? <RouterLink to="/login">Login</RouterLink></p>
  </section>
</template>
...
<style scoped>
.auth-head { background: var(--primary-red); color: #fff; }
